async function analyzeText() {
  ...
  if (data.final_label === "AI") {
    displayScore = Math.round(data.final_ai_score * 100);
    globalBox.className = "global-box global-ai";
  } else if (data.final_label === "HUMAN") {
    displayScore = Math.round((1 - data.final_ai_score) * 100);
    globalBox.className = "global-box global-human";
  } else {
    displayScore = Math.round(data.confidence * 100);
  ...
  const xlmrScore =
    Math.round((data.xlmr_ai_score || 0) * 100);

  const xlmrLargeScore =
    Math.round((data.xlmr_large_ai_score || 0) * 100);

  const debertaScore =
    Math.round((data.deberta_ai_score || 0) * 100);

  const sentenceScore =
    Math.round((data.sentence_ai_score || 0) * 100);

  const styleScore =
    Math.round((data.stylometry_ai_score || 0) * 100);

  document.getElementById("xlmrBar").style.width =
    xlmrScore + "%";

  document.getElementById("xlmrLargeBar").style.width =
    xlmrLargeScore + "%";

  document.getElementById("debertaBar").style.width =
    debertaScore + "%";

  document.getElementById("sentenceBar").style.width =
    sentenceScore + "%";

  document.getElementById("styleBar").style.width =
    styleScore + "%";

  document.getElementById("xlmrValue").innerText =
    xlmrScore + "%";

  document.getElementById("xlmrLargeValue").innerText =
    xlmrLargeScore + "%";

  document.getElementById("debertaValue").innerText =
    debertaScore + "%";

  document.getElementById("sentenceValue").innerText =
    sentenceScore + "%";

  document.getElementById("styleValue").innerText =
    styleScore + "%";

  document.getElementById("xlmrLabel").innerText =
    data.xlmr_label || "---";
  ...
  document.getElementById("avgWord").innerText =
    s.average_word_length ?? "---";

  document.getElementById("repetition").innerText =
    s.repetition_rate ?? "---";
